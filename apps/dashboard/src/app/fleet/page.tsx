      ...
      <header className="flex items-center justify-between px-5 py-2.5" style={{ background: '#0a0f1a', borderBottom: '1px solid #1e293b' }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div style={{ position: 'relative' }}>
            <button onClick={() => { setShowTokenPanel(!showTokenPanel); setTokenRevealed(false); setTokenCopied(false); }} style={{ fontSize: 10, color: '#64748b', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>
              Fleet: {report.fleetId}
            </button>
            {showTokenPanel && (
              <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 4, background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, padding: '14px 16px', zIndex: 100, boxShadow: '0 8px 24px rgba(0,0,0,.5)', width: 320 }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, color: '#94a3b8', marginBottom: 8 }}>FLEET TOKEN</div>
                <p style={{ fontSize: 11, color: '#64748b', lineHeight: 1.5, marginBottom: 12 }}>
                  This token is automatically generated by WhiteRoom to identify and track your agent activity. Use it to connect new agents to this fleet.
                </p>
                {!tokenRevealed ? (
                  <button onClick={() => setTokenRevealed(true)} style={{ fontSize: 11, color: '#38E1FF', background: 'transparent', border: '1px solid rgba(56,225,255,.3)', borderRadius: 6, padding: '6px 14px', cursor: 'pointer', width: '100%' }}>
                    Reveal Token
                  </button>
                ) : authKey ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <code style={{ flex: 1, fontSize: 11, color: '#e2e8f0', fontFamily: FONT_MONO, wordBreak: 'break-all', background: '#070B14', padding: '6px 8px', borderRadius: 4, border: '1px solid #1e293b' }}>{authKey}</code>
                    <button onClick={() => { navigator.clipboard.writeText(authKey); setTokenCopied(true); }} style={{ fontSize: 10, color: tokenCopied ? '#22c55e' : '#38E1FF', background: 'transparent', border: `1px solid ${tokenCopied ? '#166534' : 'rgba(56,225,255,.3)'}`, borderRadius: 4, padding: '4px 10px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      {tokenCopied ? 'Copied' : 'Copy'}
                    </button>
                  </div>
                ) : (
                  <p style={{ fontSize: 11, color: '#ef4444' }}>No token found in this session.</p>
                )}
              </div>
            )}
          </div>
